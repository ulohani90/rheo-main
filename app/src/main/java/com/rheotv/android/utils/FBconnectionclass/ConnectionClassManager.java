...
package com.rheotv.android.utils.FBconnectionclass;
...
public class ConnectionClassManager {
    ...
    private static String currNetwork = "UNKNOWN";
    /**
    ...
    public synchronized void netChanged() {

        try {
            String changedNetwork = NetworkUtils.getNetworkGeneration(RheoTvApp.getNonUiContext());
            if (!(changedNetwork.equals("UNKNOWN"))) {
                if (!(currNetwork.equals(changedNetwork))) {
                    //SaavnLog.i("Ramesh reset fbconn:", changedNetwork);
                    currNetwork = changedNetwork;
                    reset();
                }
            }
    ...
    private boolean significantlyOutsideCurrentBand() {
        ...
        if (average > topOfBand) {
            return average > topOfBand * HYSTERESIS_TOP_MULTIPLIER;
        } else return average < bottomOfBand * HYSTERESIS_BOTTOM_MULTIPLIER;
    }
    ...
    /**
     * Interface for listening to when {@link }
     * changes state.
     */
    public interface ConnectionClassStateChangeListener {
        /**
         * The method that will be called when {@link }
