...
public class ViewPagerMediator {
    private int startPosition;
    private ViewPager2 viewPager;
    private CustomFragmentStateAdapter adapter;
    private FragmentOnPageChangeCallback onPageChangeCallback;
    private ViewPagerOnPageSelectedListener pageChangeListener;

    public ViewPagerMediator(@NonNull ViewPager2 viewPager,
                             @NonNull StreamPlayerAdapter adapter,
                             int startPosition, ViewPagerOnPageSelectedListener pageChangeListener
    ) {
     ...
     */
    public void attach() {
        onPageChangeCallback = new FragmentOnPageChangeCallback(adapter);
    ...
    public void onMessageEvent(PageChange playerState) {
        if (playerState == PageChange.NEXT) {
    ...
    public void updateList(PostObject post) {
        ...
        if (post.isPublished()) {
            adapter.addListItem(post, viewPager.getCurrentItem() < adapter.getItemCount() ? viewPager.getCurrentItem() : 0);
        } else {
    ...
    private void showPlayRequests(int position) {
        ...
        if (fragment instanceof StreamPlayerFragment)
            ((StreamPlayerFragment) fragment).showCustomRoomAcceptedEnable(message);
    }

    private void addEvent(int position) {
        Fragment fragment = adapter.createFragment(position);
        if (fragment instanceof StreamPlayerFragment)
            ((StreamPlayerFragment) fragment).addCustomRoomEvent();
    }
