...
import com.rheotv.android.databinding.FragmentStreamPlayerV2Binding;
...
import com.rheotv.android.ui.activities.player.activity.newPlayer.adapter.TopThreeFansAdapter;
...
import com.rheotv.android.ui.decorators.RightOverlapDecorator;
...
public class StreamPlayerFragmentV3 extends BaseFragment<FragmentStreamPlayerV2Binding, MomentsFragmentViewModel>
    ...
    private FragmentStreamPlayerV2Binding mBinding;
    ...
    public MomentsListItem moments;
    ...
    public static StreamPlayerFragmentV3 getInstance(MomentsListItem postObject, String sourceScreen) {
        StreamPlayerFragmentV3 fragment = new StreamPlayerFragmentV3();
    ...
    public int getLayoutId() {
        ...
        return R.layout.fragment_stream_player_v2;
    }
    ...
    public MomentsFragmentViewModel getViewModel() {
        if (mViewModel == null) {
            mViewModel = new ViewModelProvider(this, mViewModelFactory)
                    .get("MomentFullVideo", MomentsFragmentViewModel.class);
...
//        mViewModel.loadDailyRewards();
    ...
    private void setupView() {
        if (postObject == null) return;
        mViewModel.postObject.set(postObject);
        mViewModel.getMoments().set(moments);
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        if (getActivity() instanceof StreamPlayerActivity || getActivity() instanceof MomentsActivity) {
        ...
        if (getActivity() instanceof MomentsActivity) {
            mViewModel.iconVisibility.set(true);
            mBinding.gameTagButton.setImageResource(R.drawable.ic_back_arrow_thin);
        }
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        mViewModel.comments.observe(getViewLifecycleOwner(), comments -> {
            if (chatAdapter == null) return;
            chatAdapter.addItems(comments);
        });
        mViewModel.incomingComment.observe(getViewLifecycleOwner(), comment -> {
            if (chatAdapter == null) return;
            chatAdapter.addItem(comment);
        });
        mViewModel.removeChat.observe(getViewLifecycleOwner(), pair -> {
            if (chatAdapter == null) return;
            chatAdapter.removeChatItem(pair.first, pair.second);
        });
        ...
        mViewModel.topThreeFans.observe(getViewLifecycleOwner(), topFans -> {
            if (topThreeFansAdapter == null) return;
            topThreeFansAdapter.submitList(topFans);
        });
        ...
        mViewModel.callRequestObject.observe(getViewLifecycleOwner(), this::onReceivedCallRequest);
...
//                    mBinding.videoView.pausePlayer();
                    if (mViewModel != null && mViewModel.getMoments() != null && mViewModel.getMoments().get() != null) {
                        mBinding.videoView.momentsSeekStartsFrom(Math.max(mViewModel.getMoments().get().getSeekEndedAt(), mViewModel.getMoments().get().getSeekStartedAt()));
                    }
                }
...
//        chatFragment = null;
    ...
    public void destroyView() {
        if (chatAdapter != null) {
            chatAdapter.clearChatNoteList();
            chatAdapter.notifyDataSetChanged();
        }
        if (topThreeFansAdapter != null) {
            topThreeFansAdapter.submitList(new ArrayList<>());
        }
        streamHolder = null;
        mViewModel.postObject.set(null);
        mViewModel.getMoments().set(null);
        mViewModel.currentPost.set(null);
...
//            SpannableString spannableString = new SpannableString(getString(R.string.welcome_user_message, CommonUtils.getUserName()));
    ...
    private void setOnViewClickListeners() {
        ...
        mBinding.messageContainer.setOnClickListener(new BackPressUpdateClickListener() {
            ...
            public void onViewClick(@org.jetbrains.annotations.Nullable View v) {
                if (CommonUtils.isUserLoggedin())
                    ChatBoxBottomSheetDialog.Companion.newInstance(StreamPlayerFragmentV3.this, mViewModel.authorUsername()).
...
//                    }
            ...
            bottomScratchCardView.setAction(new BackPressUpdateClickListener() {
                ...
                public void onViewClick(@Nullable View v) {
                    try {
                        ...
                        scratchDialogFragment.show(StreamPlayerFragmentV3.this.getChildFragmentManager(), AppConstants.SCRATCH_FRAGMENT_TAG, StreamPlayerFragmentV3.this, suggestReward);
    ...
    private void switchChatContainer(int orientation) {
        ...
        if (orientation == Configuration.ORIENTATION_PORTRAIT) {
            constraintSet.connect(mBinding.suggestionContainer.getId(), ConstraintSet.END, mBinding.barrierActionButtons.getId(), ConstraintSet.START);

            constraintSet.connect(mBinding.chatContainer.getId(), ConstraintSet.END, mBinding.barrierActionButtons.getId(), ConstraintSet.START);
