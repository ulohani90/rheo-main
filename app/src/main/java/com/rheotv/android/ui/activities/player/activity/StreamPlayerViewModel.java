...
public class StreamPlayerViewModel extends BaseViewModel<StreamPlayerNavigator> implements Observable {
    ...
    boolean isChatSentWhenKeyboardOpened = false;
    ...
    MutableLiveData<Pair<String, String>> removeChat = new MutableLiveData<>();
    MutableLiveData<Long> updateCheckViews = new MutableLiveData<>();
    MutableLiveData<Long> reconnectChat = new MutableLiveData<>();
    public MutableLiveData<Long> totalHearts = new MutableLiveData<>();
    ...
    public StreamPlayerViewModel(DataManager dataManager, SchedulerProvider schedulerProvider) {
    ...
    private void setPinnedComment(CommentChat commentChat) {
        if (currentPost.get() == null || (commentChat != null && (commentChat.getMessage() == null || commentChat.getMessage().isEmpty()))) {
            return;
        }
        currentPost.get().setPinnedComment(commentChat);
        pinnedChat.set(commentChat);
    }
    ...
    public String authorUsername() {
        return currentPost.get() != null ? (currentPost.get() != null && currentPost.get().getAuthor() != null ? currentPost.get().getAuthor().getUser().getUsername() : "") : (postObject.get() != null && postObject.get().getAuthor() != null && postObject.get().getAuthor().getUser() != null ? postObject.get().getAuthor().getUser().getUsername() : "");
    }
    ...
    public String getShareUrl() {
        return currentPost.get() != null ? currentPost.get().getShareUrl() : "";
    }

    void loadStreamerFollowState(int userId, String username) {
    ...
    void loadPost(boolean isForLiveStatus) {
        postRetryCount++;
        ...
        getDataManager().getSpecificPostWithUid(getPostId()).enqueue(new Callback<Result>() {
            ...
            public void onResponse(Call<Result> call, Response<Result> response) {
                if (response != null && response.body() != null) {
                    ...
                    totalHearts.setValue(response.body().getHearts());
                    ...
                    if (isPageSelected && response.body().getIsLive()) {
                        chatHelper.getTotal(postId, chatHelperCallback);
                    }
    ...
    private void setGreeting(List<PostGift> postGifts) {
...
//                            listener.error();
    ...
    private int commentRetryCount = 0;
    ...
    void pinComment(CommentChat commentChat) {
     ...
     */
    private ChatHelperCallbacks chatHelperCallback = new ChatHelperCallbacks() {
        @Override
                ...
                messageHandler.post(new Runnable() {
                    ...
                    public void run() {
                        if (chatMessage.getSender() != null && !chatMessage.getSender().isEmpty()) {
                            ...
                            if (chatMessage.getMsgType().equalsIgnoreCase(MSG_PIN)) {
                                pinnedChat.set(CommentChat.getComment(chatMessage));
                                return;
                            }
                            ...
                            } else if (chatMessage.getMessage().equalsIgnoreCase(MSG_HEART)) {
                                totalHearts.setValue((totalHearts.getValue() == null ? 0 : totalHearts.getValue()) + 1);
//                          animateHeartUp();
                            } else {
        ...
        public void setUpViewersRequest() {
            if (liveCountHandler != null)
                liveCountHandler.postDelayed(() -> {
                    if (isPageSelected)
                        chatHelper.getTotal(getPostId(), this);
                }, 5000);
        }

        @Override
        public void showToast(String message) {
            new Handler(Looper.getMainLooper()).post(() -> Toast.makeText(RheoTvApp.getNonUiContext(), "" + message, Toast.LENGTH_LONG).show());
        }
    ...
    void showRewardWonMessage() {
        ...
        chatHelper.sendMessage(gson.toJson(message), AppConstants.MSG_DYNAMIC_PLAYER_ACTIONS, postId, chatHelperCallback);
    }
    ...
    HeartAnimator.HeartAnimatorInteractionListener heartAnimatorCallback = new HeartAnimator.HeartAnimatorInteractionListener() {
        ...
        public void onHeartUp() {
            Log.i(TAG, "heat_up");
            chatHelper.sendMessage(MSG_HEART, getPostId(), chatHelperCallback);
            postHeart();
        }
    ...
    private AtomicBoolean isConnectChatRequestMade = new AtomicBoolean();

    void connectChat() {
        if (getPostId() != null && !getPostId().isEmpty() && !isConnectChatRequestMade.get()) {
            isConnectChatRequestMade.set(true);
            chatHelper.setPostChatJoinTask(new WeakReference<>(chatHelperCallback), getPostId(), live.get(), getAuthorDetail().getUsername());
        }
    }

    ChatMenuOptionData getChatOptionMenuBottomSheetData(CommentChat commentChat, String username, String profilePic) {
        ...
        if (CommonUtils.isUserLoggedin()) {
            if (canComment != null && canComment.get() != null && canComment.get()) {
                ...
                if (enterComment != null && !TextUtils.isEmpty(enterComment.get())) {
                    chatHelper.sendMessage(enterComment.get(), getPostId(), chatHelperCallback, localCommentMessageCallback);
                    enterComment.set("");
    ...
    public void sendSuggestionChat(String message) {
        if (CommonUtils.isUserLoggedin()) {
            if (canComment != null && canComment.get() != null && canComment.get()) {
                ...
                Log.e(TAG, "Sending my chat from publish.");
                chatHelper.sendMessage(message, getPostId(), chatHelperCallback, localCommentMessageCallback);
                askToComment.set(false);
            } else {
                Toast.makeText(RheoTvApp.getNonUiContext(), "You are not allowed to post messages in this live stream.", Toast.LENGTH_SHORT).show();
            }
    ...
    public void onPlayRequestSend() {
        ...
        chatHelper.sendMessage(gson.toJson(response), AppConstants.MSG_DYNAMIC_PLAYER_ACTIONS, getPostId(), chatHelperCallback, localCommentMessageCallback);
    }

    public void pinComment(CommentChat chat, String messageType) {
        chatHelper.pinMessage(chat, messageType, getPostId(), chatHelperCallback);
    }

    public void sendDeletedMessage(String message, String username, String messageType) {
        chatHelper.sendDeletedMessage(message, username, getPostId(), messageType, chatHelperCallback);
    }

    public void disconnectChat() {
        chatHelper.closeConnection(postId, chatHelperCallback, getAuthorDetail().getUsername(), true);
    }
    ...
    void updateScratchCard(String rewardId, Context context) {
    ...
    void updateScratchCardStatusShown(String rewardId, String rewardType) {
...
//                    Log.i(getClass().getName(), "loadDailyRewards " + new Gson().toJson(response));
    ...
    private void populatePost(Result result) {
    ...
    void sendGreeting(Sticker sticker, String message) {
        getCompositeDisposable().add(getDataManager().buySticker(sticker.getId(), postId, message)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(next -> {
                    if (next != null) {
    ...
    void sendSticker(Sticker sticker, View view) {
    ...
    public void sendRequestPlayFragmentUpdateMessage() {
        ...
        chatHelper.sendMessage(gson.toJson(message), AppConstants.MSG_DYNAMIC_PLAYER_ACTIONS, postId, chatHelperCallback);
        //chatHelper.sendMessage(postId, AppConstants.MSG_DYNAMIC_PLAYER_ACTIONS, postId, chatHelperCallback);
    }
    ...
    private void insertFollowStatusInDB(int userId, String username, boolean followState, FollowStatusListener followStatusListener) {
        AppUtilsKt.INSTANCE.runOnIO(this, () -> {
            ...
            AppUtilsKt.INSTANCE.runOnMain(StreamPlayerViewModel.this, () -> {
                if (followStatusListener != null)
                    followStatusListener.followStatus(followState);
                return null;
            });
    ...
    private class StreamMessageHandler extends StreamHandler<CommentChat> {
