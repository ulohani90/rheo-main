...
class YouTubeOverlay(context: Context?, private val attrs: AttributeSet?) :
    ...
    private fun forwarding() {
        ...
        val position = player?.currentPosition?.plus(fastForwardRewindDuration)?.coerceAtMost(player?.duration ?: 0) ?: 0
        seekToPosition(position)
        seekListener?.seekForward(position)
    }
    ...
    private fun rewinding() {
        ...
        val position = player?.currentPosition?.minus(fastForwardRewindDuration)?.coerceAtLeast(0) ?: 0
        seekToPosition(position)
        seekListener?.seekBackward(position)
    }
